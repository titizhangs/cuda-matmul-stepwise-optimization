...
static __global__ void matmul_basic_tiled(const float *__restrict__ A,
  ...
  for (int tile_k = 0; tile_k < num_tiles_k; ++tile_k) {
    ...
    //加载A分块到共享内存
    const size_t a_col = k_base + tx;
    if (g_row < N && a_col < N) {
      const size_t a_idx = (size_t)g_row * N + a_col;
      sA[ty][tx] = A[a_idx];
    } else {
      sA[ty][tx] = 0.0f;
    }
    ...
    for (int k = ty; k < BLOCK_K; k += BLOCK_M) {
      ...
      if (b_row < N && g_col < N) {
        const size_t b_idx = b_row * N + g_col;
        sB[k][tx] = B[b_idx];
      } else {
        sB[k][tx] = 0.0f;
      }
...
void v2_matmul_basic_tiled(const float *d_A, const float *d_B, float *d_C,
                           int N) {
