...
static __global__ void MatMulRegisterTiled(const float *__restrict__ A,
  ...
  for (int tileK = 0; tileK < numTilesK; ++tileK) {
    ...
    for (int k = ty; k < kTileK; k += kBlockM) {
      ...
      // 单线程处理跨步的元素加载，以使warp内线程间访问连续内存合并。"循环规范化"（loop canonicalization）
      for (int n = 0; n < (kTileN+kBlockN-1)/kBlockN; n++) {
        const int sBOffSet = tx+n*kBlockN;
        const size_t bCol = blockIdx.x * kTileN + sBOffSet;
        if (bRow < N && bCol < N) {
          const size_t bIdx = (size_t)bRow * N + bCol;
          sB[k][sBOffSet] = B[bIdx];
        } else {
          sB[k][sBOffSet] = 0.0f;
        }
      }
...
void V7MatMulRegisterTiled(const float *dA, const float *dB, float *dC, int N) {
